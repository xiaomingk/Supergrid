...
function analyzeresults(results::Results)
    ...
    function chart(country::Symbol; plotstoragetech=:none, plotbatterycharge=false, plotbatterylevel=false, optionlist...)
        if country == :BARS
            ...
            regcost = Systemcost ./ (vec(sum(demand, dims=1)[1:end-1]) - existinghydro) * 1000
            ...
            if lr == 21
                ...
                stackedbar(["TOTAL"], collect(annualelec[displayorder,:TOTAL]')/1e3; labels=techlabels, left_margin=30px,
                    size=(600,600), line=0, tickfont=14, legendfont=14, color_palette=palette, yformatter=:plain,
        ...
        if plotstoragetech != :none
            ...
            p = plot(regstorage; size=(900,200), tickfont=16, legendfont=16, label="storage level (TWh)")
        ...
        stackedarea(HOUR, regelec; labels=techlabels, size=(900,200), line=(0.03,1,:black), tickfont=14, legendfont=14, guidefont=14,
...
function chart_energymix_scenarios(scenarios, resultsnames, resultsfile; size=(900,200), options...)
